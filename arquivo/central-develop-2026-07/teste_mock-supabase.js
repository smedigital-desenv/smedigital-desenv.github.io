...
(function () {
  ...
  var DB = {
    perfis: [
      { id: 1, email: 'admin@exemplo.local',                 nome: 'Administração SME',     tipo: 'secretaria', is_super_admin: true,  ativo: true },
  ...
  window.ACESSO_SB = {
    ...
    rpc: function (fn, args) {
      ...
      if (fn === 'minhas_permissoes') return Promise.resolve({ data: permissoesDe('admin@exemplo.local'), error: null });
      return Promise.resolve({ data: null, error: null });
    }
  };

  // Perfil (super admin) que "abre" o painel na demo.
  window.ACESSO_DEMO_PERFIL = { id: 1, nome: 'Administração SME (DEMO)', email: 'admin@exemplo.local', tipo: 'secretaria', is_super_admin: true };
